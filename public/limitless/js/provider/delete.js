$(function(){
    ...
    $('#grid_providers').on('click','.btn-delete',function(e){
        e.preventDefault();

        var id=$(this).data('id');
        var form = $('#form_delete');
        var url = form.attr('action').replace('PROVIDER_ID', id);
        var data = {'id': id};
                    ...
                    callback: function() {
                        $.post(url, data, function (result) {
                            ...
                            if(removed==1){
                                ...
                                var selectedData = gridProvidersOptions.api.getSelectedRows();
                                var res = gridProvidersOptions.api.updateRowData({remove: selectedData});
